...
quizApp.directive("iquestion", ['QuizService', function (QuizService) {
    return {
        ...
        controller: function ($scope) {
            ...
            $scope.evalScore = function (id, value, option) {
                ...
                //$scope.qna.options[arrIndex].style = "background-color:#ffff00";
                angular.forEach($scope.qna[0].options, function(value, key){
                    if(value.correct) value.style = "background-color:#ffff00";
                });
            };
